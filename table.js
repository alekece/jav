 ...
 * Entry point 
 */
function renderTable(screen, table, rows) {
    table.setData(
        {
            headers: header,
...
async function createTable(dataz) {
    ...
    bindColumnSortingKeys(screen, table, rows)

    var grid = new contrib.grid({ rows: 12, cols: 12, screen: screen })
    var table = grid.set(0, 0, 10, 12, contrib.table, styles.table({
        keys: true
        , interactive: true
    ...
    var rows = formatData(dataz)

    // allow control the table with the keyboard
    table.focus()
    render(screen, table, rows)
}
...
function bindColumnSortingKeys(screen, table, rows) {
    keyBindings.forEach(
        function (keyB) {
